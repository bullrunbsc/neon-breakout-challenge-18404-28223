...
-- Drop the old policy that allows anyone to view all round data
DROP POLICY IF EXISTS "Anyone can view rounds" ON public.rounds;

-- Create new policy that hides the correct_code field from players
-- Players can see round questions and metadata, but not answers
CREATE POLICY "Players can view round questions only"
ON public.rounds
FOR SELECT
USING (
  -- Allow selecting only non-sensitive fields
  -- The correct_code will still be in the row, but we'll handle this in the app
  true
);

-- Note: Postgres RLS can't hide specific columns, so we'll need to update 
-- the client code to never request or display correct_code